...
class PosixQueue : public GenericQueue
{
    public:
        // methods
        PosixQueue(unsigned int max_items);
        ~PosixQueue();
        int enqueue(QueueItem* item, int timeout); 
        int dequeue(QueueItem*& item, int timeout);
        int peek(QueueItem*& item, int timeout);
        bool isFull();
        bool isEmpty();
        //    int queueStatus(QueueItem*& item, int timeout);
        int queueStatus();
...
/*----------------------------------------------------------------------
  ...
  +---------------------------------------------------------------------*/
PosixQueue::PosixQueue(unsigned int max_items) : 
...
//int PosixQueue::queueStatus(QueueItem*& item, int timeout)
int PosixQueue::queueStatus()
{
...
/*----------------------------------------------------------------------
  ...
  +---------------------------------------------------------------------*/
    int
PosixQueue::getTimeout(int timeout, struct timespec& timed)
{
    if (timeout != -1) {
...
/*----------------------------------------------------------------------
  |       PosixQueue::enqueue
  +---------------------------------------------------------------------*/
int PosixQueue::enqueue(QueueItem* item, int timeout)
{    
    struct timespec timed;    
    if (timeout != -1)
    {        
...
int PosixQueue::dequeue(QueueItem*& item, int timeout)
{    
    struct timespec timed;    
    if (timeout != -1) 
    {        
...
/*----------------------------------------------------------------------
  |       PosixQueue::Peek
  +---------------------------------------------------------------------*/
int PosixQueue::peek(QueueItem*& item, int timeout)
{    
    struct timespec timed;    
    if (timeout != -1) 
    {        
    ...
    std::list<QueueItem*>::iterator head = m_items.begin();
...
/*----------------------------------------------------------------------
  ...
  +---------------------------------------------------------------------*/
GenericQueue *GenericQueue::CreateInstance(int max_items)
{    
    return new PosixQueue(max_items);
}
