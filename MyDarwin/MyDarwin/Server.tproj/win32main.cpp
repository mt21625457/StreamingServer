...
int main(int argc, char * argv[]) 
{
   ...
   char* compileType = "Compile_Flags/_RELEASE;";
    ...
    while ((ch = getopt(argc,argv, "vdxp:o:c:irsS:I")) != EOF) // opt: means requires option
    {
        switch(ch)
        {
            case 'v':
                ...
                qtss_printf("-i: Install the MyDarwin service\n");
                qtss_printf("-r: Remove the MyDarwin service\n");
                qtss_printf("-s: Start the MyDarwin service\n");
            ...
            case 'x':
                ...
                break;
            case 'i':
                qtss_printf("Installing the MyDarwin service...\n");
                ::InstallService("MyDarwin");
                qtss_printf("Starting the MyDarwin service...\n");
                ::RunAsService("MyDarwin");
                ::exit(0);
                break;
            case 'r':
                qtss_printf("Removing the MyDarwin service...\n");
                ::RemoveService("MyDarwin");
                ::exit(0);
            case 's':
                qtss_printf("Starting the MyDarwin service...\n");
                ::RunAsService("MyDarwin");
    ...
    qtss_printf("MyDarwin must either be started from the DOS Console\n");
    qtss_printf("using the -d command-line option, or using the Service Control Manager\n\n");
    qtss_printf("Waiting for the Service Control Manager to start MyDarwin...\n");
...
void InstallService(char* inServiceName)
{
    ...
    if (!theSCManager)
    {
        qtss_printf("Failed to install MyDarwin Service\n");
        return;
    }
    ...
    {
        ::CloseServiceHandle(theService);
        qtss_printf("Installed MyDarwin Service\n");
    }
    else
        qtss_printf("Failed to install MyDarwin Service\n");
...
void RemoveService(char *inServiceName)
{
    ...
    if (!theSCManager)
    {
        qtss_printf("Failed to remove MyDarwin Service\n");
        return;
    }
    ...
    if (theService != NULL)
    {
        ...
        ::CloseServiceHandle(theService);
        qtss_printf("Removed MyDarwin Service\n");
    }
    else
        qtss_printf("Failed to remove MyDarwin Service\n");
