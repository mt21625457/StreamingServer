...
int main(int argc, char * argv[]) 
{
	...
	static char* sDefaultConfigFilePath = DEFAULTPATHS_ETC_DIR_OLD "mydarwin.conf";
	static char* sDefaultXMLFilePath = DEFAULTPATHS_ETC_DIR "mydarwin.xml";
    ...
    while ((ch = getopt(argc,argv, "vdfxp:DZ:c:o:S:Ih")) != EOF) // opt: means requires option arg
    {
        switch(ch)
        {
            ...
            case 'Z':
                ...
                break;
            case 'f':
				theXMLFilePath  = DEFAULTPATHS_ETC_DIR "MyDarwin.xml";
                break;
            case 'p':
                Assert(optarg != NULL);// this means we didn't declare getopt options correctly or there is a bug in getopt.
                thePort = ::atoi(optarg);
...
//		if (_daemonize(_DF_NOFORK, STDIN_FILENO, STDOUT_FILENO, STDERR_FILENO) != 0)
    ...
    //TODO(xuhuichi) add by xuhuichi 屏敝SIGPIPE信号,需要确认为什么sigaction不起作用
    sigset_t newmask;
    sigemptyset(&newmask);
